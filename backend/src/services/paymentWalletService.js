...
const { getDecimals } = require('../config/assets');
 ...
 */
function toAssetAmount(assetSymbol, amountStr) {
    const decimals = getDecimals(assetSymbol);
    const n = BigInt(amountStr);
    const factor = 10n ** BigInt(decimals);
    return Number(n) / Number(factor);
}
...
async function attachWalletToPayment(paymentId) {
  ...
  const assetSymbol = payment.assetSymbol;
  ...
  const assetAmount = toAssetAmount(assetSymbol, payment.amount);
  const usdAmount =
