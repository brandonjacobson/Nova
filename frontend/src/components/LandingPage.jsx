...
const LandingPage = () => {
  ...
  // Feature card visibility state for CSS animation
  const [visibleCards, setVisibleCards] = useState(new Set());
  const cardRefs = useRef([]);
  const sectionHeaderRef = useRef(null);
  const footerRef = useRef(null);
  const [sectionHeaderVisible, setSectionHeaderVisible] = useState(false);
  const [footerVisible, setFooterVisible] = useState(false);

  // IntersectionObserver for feature cards and section/footer reveals
  useEffect(() => {
      ...
      <div className="content-container">
        ...
        <section className="features-section">
          <motion.div
            ...
            initial={{ opacity: 0, y: 20 }}
            animate={sectionHeaderVisible ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
        ...
        <motion.footer
          className="footer"
          initial={{ opacity: 0 }}
          animate={footerVisible ? { opacity: 1 } : { opacity: 0 }}
          transition={{ duration: 0.6 }}
          ref={footerRef}
        >
